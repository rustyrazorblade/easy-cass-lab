 ...
 * - **User IAM Policies**: Policies users need to create easy-cass-lab resources
...
sealed class AWSPolicy {
    ...
    sealed class Trust : AWSPolicy() {
         ...
         * Used by: EasyCassLabEC2Role, EasyCassLabEMREC2Role
         */
         ...
         * Used by: EasyCassLabEMRServiceRole
         */
    ...
    /**
     * User IAM policies that users must attach to their AWS account to use easy-cass-lab.
     * These policies are loaded from JSON resource files with account ID substitution.
     *
     * Three policies are required:
     * 1. EasyCassLabEC2 - EC2 operations and PassRole
     * 2. EasyCassLabIAM - IAM and S3 management
     * 3. EasyCassLabEMR - EMR cluster operations
     */
        ...
        companion object {
            ...
            fun loadAll(accountId: String): List<Policy> {
                ...
                return policyData.map { (fileName, policyName) ->
                    val policyStream =
                        User::class.java.getResourceAsStream("/com/rustyrazorblade/easycasslab/$fileName")
                    val policyContent =
    ...
    sealed class Inline : AWSPolicy() {
        ...
        /**
         * S3 bucket policy granting access to all three easy-cass-lab IAM roles.
