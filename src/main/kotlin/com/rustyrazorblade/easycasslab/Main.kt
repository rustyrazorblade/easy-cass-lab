    ...
    try {
        ...
        if (e.statusCode() == Constants.HttpStatus.FORBIDDEN) {
            ...
            with(TermColors()) {
                ...
                println(red("\nYou need to add the EasyCassLabEC2 IAM policy which includes S3 permissions."))
        ...
        if (e.statusCode() == Constants.HttpStatus.FORBIDDEN) {
            ...
            with(TermColors()) {
                ...
                println(red("\nYou need to add the EasyCassLabEC2 IAM policy which includes EC2 permissions."))
                println(yellow("\nFor detailed error information, check the logs (logs/info.log by default)"))
            }
            exitProcess(Constants.ExitCodes.ERROR)
        }
