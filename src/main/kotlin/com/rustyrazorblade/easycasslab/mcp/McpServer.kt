...
class McpServer(private val context: Context) : KoinComponent {
     ...
     */
    private fun createStatusHandler(): (CallToolRequest) -> CallToolResult = { _ ->
     ...
     */
    private fun createToolHandler(toolName: String, inputSchema: JsonObject): (CallToolRequest) -> CallToolResult = { request ->
        // Try to acquire semaphore without blocking
        if (!executionSemaphore.tryAcquire()) {
            // Another command is already running
            CallToolResult(
                content = listOf(
                    TextContent(text = "Another tool is already running. Please wait for it to complete or check status with 'get_status'.")
                ),
                isError = false
            )
        } else {
