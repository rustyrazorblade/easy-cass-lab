...
interface DockerClientInterface {
    fun listImages(name: String, tag: String): List<Image>
    fun pullImage(name: String, tag: String, callback: PullImageResultCallback)
    fun createContainer(imageTag: String): ContainerCreationCommand
    fun attachContainer(containerId: String, stdin: PipedInputStream, callback: ResultCallback.Adapter<Frame>)
    fun startContainer(containerId: String)
    fun inspectContainer(containerId: String): InspectContainerResponse
    fun removeContainer(containerId: String, removeVolumes: Boolean)
}
...
class DefaultDockerClient(private val dockerClient: DockerClient) : DockerClientInterface {
    override fun listImages(name: String, tag: String): List<Image> {
        return dockerClient.listImagesCmd().withImageNameFilter("$name:$tag").exec()
    }
    
    override fun pullImage(name: String, tag: String, callback: PullImageResultCallback) {
        val pullCommand = if (tag.isNotBlank()) {
    ...
    override fun attachContainer(containerId: String, stdin: PipedInputStream, callback: ResultCallback.Adapter<Frame>) {
    ...
    override fun removeContainer(containerId: String, removeVolumes: Boolean) {
...
class Docker(
    ...
    private fun buildReturnMessage(containerState: InspectContainerResponse.ContainerState, framesRead: Int): String {
        var errorMessage = ""
