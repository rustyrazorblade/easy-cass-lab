...
class FilteringChannelOutputHandlerTest {
    ...
    @Test
    fun `should drop frame events and not send them to channel`() = runBlocking {
    ...
    @Test
    fun `should send docker activity message every 100 frames`() = runBlocking {
        val channel = Channel<OutputEvent>(Channel.UNLIMITED)
        val handler = FilteringChannelOutputHandler(channel)
    ...
    @Test
    fun `should send docker activity message every 100 frames with multiple batches`() = runBlocking {
        val channel = Channel<OutputEvent>(Channel.UNLIMITED)
        val handler = FilteringChannelOutputHandler(channel)
    ...
    @Test
    fun `should pass through MessageEvent unchanged`() = runBlocking {
        val channel = Channel<OutputEvent>(Channel.UNLIMITED)
        val handler = FilteringChannelOutputHandler(channel)
    ...
    @Test
    fun `should pass through ErrorEvent unchanged`() = runBlocking {
        val channel = Channel<OutputEvent>(Channel.UNLIMITED)
        val handler = FilteringChannelOutputHandler(channel)
    ...
    @Test
    fun `should pass through ErrorEvent with null throwable`() = runBlocking {
        val channel = Channel<OutputEvent>(Channel.UNLIMITED)
        val handler = FilteringChannelOutputHandler(channel)
    ...
    @Test
    fun `should send CloseEvent when close is called`() = runBlocking {
        val channel = Channel<OutputEvent>(Channel.UNLIMITED)
        val handler = FilteringChannelOutputHandler(channel)
    ...
    @Test
    fun `should handle mixed event types correctly`() = runBlocking {
        val channel = Channel<OutputEvent>(Channel.UNLIMITED)
        val handler = FilteringChannelOutputHandler(channel)
