 ...
 *
 * This class ensures that critical services (like AWS) are always mocked to prevent accidental real
 * API calls during testing, while allowing tests to add their own specific modules as needed.
 *
 * Usage:
...
abstract class BaseKoinTest : KoinTest {
    ...
    /**
     * Core test modules that should always be loaded. These include mocks for services that should
    ...
    /**
     * Set up Koin before each test. Combines core modules with any additional test-specific
     * modules.
     */
    @BeforeEach
    fun setupKoin() {
        ...
        startKoin { modules(coreTestModules() + additionalTestModules()) }
    }

    /** Tear down Koin after each test. Ensures clean state between tests. */
    @AfterEach
    fun tearDownKoin() {
        stopKoin()
    }
